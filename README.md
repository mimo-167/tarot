...
- 收藏牌阵和同日固定的每日一牌。
- 游客可完成抽牌和本地解读；游客调用 AI 时，服务端只向浏览器返回完整的“逐张看牌”和“这组牌连起来看”的前半段，其余组合分析、问题答案与行动提醒不会进入游客响应，邮箱登录后可免费解锁本次完整解读。
- 即时自伤、伤人或人身危险等危机安全回应不受游客预览限制，服务端会完整展示求助指引而不要求登录。
- Resend 中英文邮箱验证码登录：验证码短时有效、限制尝试次数，登录态使用 `HttpOnly` Cookie；登录后会接续同一游客会话中的 AI 解读。
- 游客翻完牌后不会出现保存或登录弹窗；登录用户翻完牌后会询问是否保存，可保存问题、牌阵、牌面快照和 AI 解读，并在“我的抽牌记录”查看或删除。
...
- 抽牌、翻牌、本地解读和 AI 解读正常。
- 游客 AI 响应只包含完整的逐张解读与组合分析前半段，且不包含“回到你的问题”及后续正文；登录后能够解锁同一次完整解读。
- 游客翻完牌后不出现保存或登录弹窗；登录用户的保存询问、记录查看与删除正常。
